...
export default function PomodoroPage(){
     ...
     const {text, circleType, volume, time, pattern, prevCircleType} = exerciseState
     useEffect(() => {
          if (!time) return;
     ...
     }, [t, pattern, growTime, holdTime]);
     const holdCircle = cn(
          "animate-hold-circle",
          circleType === "hold" &&
               prevCircleType === "shrinking" &&
                    "scale-[0.25]",
          circleType === "hold" &&
               prevCircleType === "growing" &&
                    "scale-100"
     );
     return (
          <WindowWrapper title={t("title")} className="space-y-2 flex justify-center items-center breathing-root">
               <div className="bg-card/40 backdrop-blur-sm text-card-foreground border shadow-xs rounded-md p-6 flex justify-center items-center flex-col gap-4 max-w-[400px] relative group">
                    <div className="size-72 flex justify-center items-center rounded-full will-change-transform">
                    ...
                    <div className="flex items-center gap-2 w-full">
                         {volumeIcon}
                         <Slider defaultValue={[volume]} min={0} max={100} onValueChange={(newVolume)=>setState({ volume: newVolume[0] })}/>
                    </div>
               </div>
          </WindowWrapper>
     )
